...
describe('ZkappCommandsService', () => {
  describe('normalizeZkappCommandRange', () => {
    ...
    test('caps from-only ranges to the configured maximum', () => {
      assert.deepStrictEqual(normalizeZkappCommandRange({ from: 5 }), {
        from: 5,
        to: 5 + BLOCK_RANGE_SIZE,
      });
    });

    test('caps to-only ranges to the configured maximum', () => {
      assert.deepStrictEqual(
        normalizeZkappCommandRange({ to: 5 + BLOCK_RANGE_SIZE }),
        {
          from: 5,
          to: 5 + BLOCK_RANGE_SIZE,
        }
    ...
    test('rejects ranges over the configured maximum', () => {
      assert.throws(() =>
        normalizeZkappCommandRange({ from: 0, to: BLOCK_RANGE_SIZE + 1 })
      );
    });

    test('allows a range exactly at the configured maximum', () => {
      assert.deepStrictEqual(
        normalizeZkappCommandRange({ from: 0, to: BLOCK_RANGE_SIZE }),
        {
          from: 0,
          to: BLOCK_RANGE_SIZE,
        }
