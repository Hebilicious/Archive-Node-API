  ...
  return dbClient<ZkappCommandDatabaseRow[]>`
    ...
    block_range AS (
      ...
      WHERE
        ${
          from !== undefined
            ? dbClient`b.height >= ${from}`
            : dbClient`b.height >= (SELECT value FROM max_height) - ${BLOCK_RANGE_SIZE}`
        }
        ${to !== undefined ? dbClient`AND b.height < ${to}` : dbClient``}
        ${
    ...
    SELECT
      ...
      zkc.memo,
      bzkc.status,
      bzkc.failure_reasons_ids::text AS failure_reason,
